...
class usercontrol extends base {
	...
	function onuploadavatar() {
		...
		$tmpavatar = SAE_STORAGE.'./tmp/upload'.$uid.$filetype;
		$tmpavatar_sae = SAE_STORAGE_FILE.'/tmp/upload'.$uid.$filetype;
		$storage->fileExists(SAE_STORAGE_NAME,$tmpavatar_sae) && $storage->delete(SAE_STORAGE_NAME,$tmpavatar_sae);
		if(@copy($_FILES['Filedata']['tmp_name'], $tmpavatar) || @move_uploaded_file($_FILES['Filedata']['tmp_name'], $tmpavatar)) {
			@unlink($_FILES['Filedata']['tmp_name']);
			list($width, $height, $type, $attr) = getimagesize($tmpavatar);
			if($width < 10 || $height < 10 || $type == 4) {
				$storage->delete(SAE_STORAGE_NAME,$tmpavatar_sae);
				return -2;
			}
		} else {
			@unlink($_FILES['Filedata']['tmp_name']);
		...
		$storage = new SaeStorage();
		$avatarurl = $storage->getUrl(SAE_STORAGE_NAME,SAE_STORAGE_FILE.'/tmp/upload'.$uid.$filetype);
		return $avatarurl;
	}
	...
	function onrectavatar() {
		...
		if(!is_dir(SAE_STORAGE.'./avatar/'.$home)) {
			$this->set_home($uid, SAE_STORAGE.'./avatar/');
		}
		$avatartype = getgpc('avatartype', 'G') == 'real' ? 'real' : 'virtual';
		$bigavatarfile = SAE_STORAGE.'./avatar/'.$this->get_avatar($uid, 'big', $avatartype);
		$middleavatarfile = SAE_STORAGE.'./avatar/'.$this->get_avatar($uid, 'middle', $avatartype);
		$smallavatarfile = SAE_STORAGE.'./avatar/'.$this->get_avatar($uid, 'small', $avatartype);
		$bigavatarfile_sae = SAE_STORAGE_FILE.'./avatar/'.$this->get_avatar($uid, 'big', $avatartype);
		$middleavatarfile_sae = SAE_STORAGE_FILE.'./avatar/'.$this->get_avatar($uid, 'middle', $avatartype);
		$smallavatarfile_sae = SAE_STORAGE_FILE.'./avatar/'.$this->get_avatar($uid, 'small', $avatartype);
		...
		if(!$biginfo || !$middleinfo || !$smallinfo || $biginfo[2] == 4 || $middleinfo[2] == 4 || $smallinfo[2] == 4
			...
			$storage->fileExists(SAE_STORAGE_NAME,$bigavatarfile_sae) && $storage->delete(SAE_STORAGE_NAME,$bigavatarfile_sae);
			$storage->fileExists(SAE_STORAGE_NAME,$middleavatarfile_sae) && $storage->delete(SAE_STORAGE_NAME,$middleavatarfile_sae);
			$storage->fileExists(SAE_STORAGE_NAME,$smallavatarfile_sae) && $storage->delete(SAE_STORAGE_NAME,$smallavatarfile_sae);
			$success = 0;
		}

		$filetype = '.jpg';
		$storage->delete(SAE_STORAGE_NAME,SAE_STORAGE_FILE.'/tmp/upload'.$uid.$filetype);
