...
class usermodel {
	...
	function delete_useravatar($uidsarr) {
		...
		foreach((array)$uidsarr as $uid) {
			$uid_big_real = SAE_STORAGE_FILE.'/avatar/'.$this->base->get_avatar($uid, 'big', 'real');
			$storage->fileExists(SAE_STORAGE_NAME,$uid_big_real) && $storage->delete(SAE_STORAGE_NAME,$uid_big_real);
			
			$uid_middle_real = SAE_STORAGE_FILE.'/avatar/'.$this->base->get_avatar($uid, 'middle', 'real');
			$storage->fileExists(SAE_STORAGE_NAME,$uid_middle_real) && $storage->delete(SAE_STORAGE_NAME,$uid_middle_real);
			
			$uid_small_real = SAE_STORAGE_FILE.'/avatar/'.$this->base->get_avatar($uid, 'small', 'real');
			$storage->fileExists(SAE_STORAGE_NAME,$uid_small_real) && $storage->delete(SAE_STORAGE_NAME,$uid_small_real);
			
			$uid_big = SAE_STORAGE_FILE.'/avatar/'.$this->base->get_avatar($uid, 'big');
			$storage->fileExists(SAE_STORAGE_NAME,$uid_big) && $storage->delete(SAE_STORAGE_NAME,$uid_big);
			
			$uid_middle = SAE_STORAGE_FILE.'/avatar/'.$this->base->get_avatar($uid, 'middle');
			$storage->fileExists(SAE_STORAGE_NAME,$uid_middle) && $storage->delete(SAE_STORAGE_NAME,$uid_middle);
			
			$uid_small = SAE_STORAGE_FILE.'/avatar/'.$this->base->get_avatar($uid, 'small');
			$storage->fileExists(SAE_STORAGE_NAME,$uid_small) && $storage->delete(SAE_STORAGE_NAME,$uid_small);
		}
	}
