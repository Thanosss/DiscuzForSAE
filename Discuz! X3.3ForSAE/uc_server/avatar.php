...
*/
define('SAE_MEMCACHE', 'saemc://uc_server/');
define('SAE_STORAGE_NAME', 'discuzx');
define('SAE_STORAGE_FILE', 'uc_server');
define('SAE_STORAGE', 'saestor://'.SAE_STORAGE_NAME.'/'.SAE_STORAGE_FILE.'/');
...
$avatar_sae = str_replace(array('./data'),SAE_STORAGE_FILE,$avatar);
if($storage->fileExists(SAE_STORAGE_NAME,$avatar_sae)) {
	if($check) {
		echo 1;
		exit;
	}
	$random = !empty($random) ? rand(1000, 9999) : '';
	$avatar_url = $storage->getCDNUrl(SAE_STORAGE_NAME,$avatar_sae);
	$avatar_url_sae = empty($random) ? $avatar_url : $avatar_url.'?random='.$random;
} else {
	if($check) {
		echo 0;
		exit;
