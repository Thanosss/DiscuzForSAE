...
function template($file, $templateid = 0, $tpldir = '', $gettplfile = 0, $primaltpl='') {
	...
	if(strpos($file, ':') !== false) {
		...
		if($templateid == 'diy') {
			...
			$_G['style']['prefile'] = '';
			$diypath = SAE_STORAGE.'./data/diy/'.$_G['style']['tpldirectory'].'/'; //DIY模板文件目录
			$preend = '_diy_preview';
	...
	checktplrefresh($tplfile, $tplfile, @filemtime(SAE_MEMCACHE .$cachefile), $templateid, $cachefile, $tpldir, $file);
	return SAE_MEMCACHE.$cachefile;
}
