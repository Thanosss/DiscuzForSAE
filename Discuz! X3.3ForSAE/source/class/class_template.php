...
class template {
	...
	function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
		...
		if(!@$fp = fopen(SAE_MEMCACHE .$cachefile, 'w')) {
	...
	function loadcsstemplate() {
		...
		$content = @implode('', file(SAE_STORAGE.'./data/cache/style_'.STYLEID.'_module.css'));
		...
		if($this->csscurmodules) {
			...
			if(@$fp = fopen(SAE_STORAGE.'./data/cache/style_'.STYLEID.'_'.$_G['basescript'].'_'.CURMODULE.'.css', 'w')) {
