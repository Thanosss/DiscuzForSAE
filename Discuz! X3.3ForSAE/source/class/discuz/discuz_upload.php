...
Class discuz_upload{
	...
	function init($attach, $type = 'temp', $extid = 0, $forcename = '') {
		...
		if(!is_array($attach) || empty($attach) || !$this->is_upload_file($attach['tmp_name']) || trim($attach['name']) == '' || $attach['size'] == 0) {
			...
			$attach['target'] = str_replace(array(DISCUZ_ROOT),SAE_STORAGE,getglobal('setting/attachdir').'./'.$this->type.'/'.$attach['attachment']);
			$this->attach = & $attach;
			$this->errorcode = 0;
			return true;
		}
	...
	function check_dir_exists($type = '', $sub1 = '', $sub2 = '') {
		...
		$basedir = !getglobal('setting/attachdir') ? (SAE_STORAGE.'./data/attachment') : getglobal('setting/attachdir');
